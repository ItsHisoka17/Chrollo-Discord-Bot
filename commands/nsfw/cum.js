...
module.exports = class Cum extends BaseCommand {
    constructor(client){
    super(client, {
    name: 'cum',
    category: 'NSFW',
...
async execute(message, client, args) {
    ...
    let result = await fetch.get('https://nekos.life/api/v2/img/cum')
    message.channel.send(new MessageEmbed() 
    .setImage(result.body.url)
    .setColor(require('../../utils/utils').getRandom(['0xfff8f7', '#eba0c4']))
    .setFooter(`${message.author.username} Cums` ,message.author.displayAvatarURL({dynamic: true}))
    )
}
}